...
[lead]
This message occurs when a new node that is trying to join the cluster is using the same IP address that is configured on this node's cluster interface.
